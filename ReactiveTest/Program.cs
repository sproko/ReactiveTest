...
namespace ReactiveTest
{
    internal static class Program
    {
        public static async Task Main(string[] args)
        {
            ...
            // Publish Commands
            Console.WriteLine($"[{DateTime.Now.Pretty()}][MAIN] Sending Open Command to Shutter {sh1} ");
            messageBus.Publish(new CommandOpenShutter(sh1));

            Console.WriteLine($"[{DateTime.Now.Pretty()}][MAIN] Sending Open Command to Shutter {sh2} ");
            messageBus.Publish(new CommandOpenShutter(sh2));

            Console.WriteLine($"[{DateTime.Now.Pretty()}][MAIN] Waiting for Shutter {sh2} to open with shutter reference");
            await shutter2.WaitForSensorStateAsync(TimeSpan.FromSeconds(3));

            // Close shutters
            Console.WriteLine($"[{DateTime.Now.Pretty()}][MAIN] Sending Close Command to Shutters");
            messageBus.Publish(new CommandCloseShutter(sh1));
            messageBus.Publish(new CommandCloseShutter(sh2));

            Console.WriteLine($"[{DateTime.Now.Pretty()}][MAIN] Waiting for Shutter {sh1} to close");
            await shutter2.WaitForSensorStateAsync(TimeSpan.FromSeconds(3));
            ...
            Console.WriteLine($"[{DateTime.Now.Pretty()}][MAIN] [Shutter {response.ShutterId}] is {response.State}");

            Console.ReadKey();
            shutter1.Dispose();
            shutter1 = null;

            Console.WriteLine($"[{DateTime.Now.Pretty()}][MAIN] got rid of shutter 1 {sh1} .. opening shutter {sh2} (should only see Notifications From Shutter 2)");
            await shutter2.OpenShutterAsync();
            Console.WriteLine($"[{DateTime.Now.Pretty()}][MAIN] Shutter 2 {sh2} should be opened with shutter  ");
